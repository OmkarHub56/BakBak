...
//        currentUser=database.getReference().child("Users").child(auth.getCurrentUser().getUid());
        ...
        database.getReference().child(ConstantsClass.USERS).child(currentUserUId).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(resultCode==RESULT_OK){
            if (requestCode==SELECT_IMAGE_CODE){
                ...
                ref.putFile(uri).addOnSuccessListener(taskSnapshot -> ref.getDownloadUrl().addOnSuccessListener(uri1 -> database.getReference().child(ConstantsClass.USERS).child(currentUserUId).child(ConstantsClass.PROFILE_PIC).setValue(uri1.toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
